...
```bash
echo "JWT_SECRET_KEY=$(openssl rand -hex 32)" >> .env.example
echo "SECRET_KEY=$(openssl rand -hex 32)" >> .env.example
echo "BOT_API_KEY=$(openssl rand -hex 32)" >> .env.example
```
...
```bash
sudo -u discord nano /opt/discord-bot/.env.example
```
